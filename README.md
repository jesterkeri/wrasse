...
`rapport/memory_gate.py`. If Sibyl raises an error, Rapport raises
...
Verified Base outcomes are written in `rapport/evidence.py`, after the receipt,
chain, contract, event signature, deal, provider, and final state are checked in
`rapport/reconciler.py`. The WARM `chain_event/<event_id>` entity is the
...
```bash
uv run rapport recall <provider-address>
uv run rapport reconcile-timeout <tx-hash> --deal-id <id> --provider <address>
uv run rapport learn-dimension <event-id>
uv run rapport policy <provider-address> --output policy.json
```

Copy `.env.example` to the gitignored `.env` and add a project-specific
`OPENROUTER_API_KEY`. `RAPPORT_LLM_MODEL` is configurable and defaults to
`openai/gpt-oss-20b`.

## Base contract

`RapportEscrow` derives `policyHash` onchain from the provider, actual
...
```bash
cd contracts
forge script script/Deploy.s.sol:DeployRapportEscrow \
  --rpc-url https://sepolia.base.org \
  --broadcast \
  --keystore ../.rapport/keystores/rapport-deployer \
  --password-file ../.rapport/keystore.password
```
